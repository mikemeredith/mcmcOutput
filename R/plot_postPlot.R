...
postPlot1 <- function(x, credMass, compVal, ROPE, HDItextPlace, showMode, showCurve,
  ...
  } else if(diff(range(x)) < sqrt(.Machine$double.eps)) {
  ...
  if(is.null(useArgs$xlim))
    useArgs$xlim <- range(compVal, hdi(x, 0.99))

  if (is.null(breaks)) {
    if (all(x == round(x))) { # all integers
      breaks <- seq(min(x), max(x) + 1) - 0.5
    } else {
      nbreaks <- ceiling(diff(range(x)) /
                          diff(hdi(x)) * 18)
      breaks <- seq( from=min(x), to=max(x),
                     length.out=nbreaks)
    }
  }
  histinfo <- hist(x, breaks=breaks, plot=FALSE)
  histinfo$xname <- useArgs$xlab

  if (showCurve) {
    densCurve <- densityFolded(x, adjust=2)
    ...
    if(!is.null(credMass)) {
      ...
      if(nrow(HDI) == 1)  # hdi is not split
        HDI <- matrix(hdi(x, credMass), nrow=1)
      if(!is.null(shadeHDI))  {
        for (i in 1:nrow(HDI)) {
  ...
  if (!is.null(compVal)) {
    cvHt <- 0.8 * cenTendHt
    cvCol <- "darkgreen"
    pcgtCompVal <- round(100 * mean(x > compVal))
  ...
  if ( !is.null( ROPE ) ) {
    ...
    ropeCol <- "darkred"
    pcInROPE <- mean(x > ROPE[1] & x < ROPE[2])
...
postPlot <- function(object, credMass=0.95, compVal=NULL, ROPE=NULL,
  ...
  if(is.null(dim(object))) {
    ...
    colnames(object) <- objName
  }
  # convert a multi-d array to a matrix
  if(length(dim(object)) > 2)
    object <- arrayToMat(object, name=objName)
  ...
  if(!is.numeric(object))
    stop("The first argument must be numeric.")
  nPlots <- ncol(object)
  ...
  for(i in 1:nPlots) {
    if(is.null(dots$xlab))
      useArgs$xlab <- colnames(object)[i]
    postPlot1(object[, i], credMass=credMass, compVal=compVal, ROPE=ROPE,
        HDItextPlace=HDItextPlace, showMode=showMode, showCurve=showCurve,
             shadeHDI=shadeHDI, useArgs=useArgs, breaks=breaks)
    title(objName, outer=TRUE, cex.main=useArgs$cex.main)
  }
}
